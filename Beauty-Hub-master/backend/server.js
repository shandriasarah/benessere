...
// --- ROTAS DE AUTENTICAÇÃO (DIRETO NO APP PARA TESTE) ---

app.post("/api/auth/register", async (req, res) => {
  const { name, email, password } = req.body;
  const pool = db.getPool(); // Pega a conexão correta
  ...
  try {
    ...
    pool.query(sql, [name, email, hashPassword], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Erro ao cadastrar. E-mail já existe." });
      }
...
app.post("/api/auth/login", (req, res) => {
  ...
  pool.query(sql, [email], async (err, data) => {
    ...
    const token = jwt.sign({ id: data[0].id }, "SUA_CHAVE_SECRETA", {
      expiresIn: "1d",
    });
